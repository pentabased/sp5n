...
the shuttle wheel translates physical key events into bends — one
...
a splice-based input source has different constraints. it produces
...
the shuttle wheel and the splice wheel are two ways to feed thread
onto the loom. the distinction is about input granularity, not about
who's using it — a human could use a splice wheel (e.g. dictation),
and an animal could use a shuttle wheel.

```
shuttle wheel (petals)           splice wheel (neems/phrases)
       │                                │
...
the shuttle wheel's atomic unit is a single bloom bend. the shuttle
cursor is essential — it tells the operator where they are and what
scope they're operating at.

the splice wheel's atomic units are neems and phrases. the shuttle is
not useful — the operator can specify exactly where to act by
addressing positions in the document tree directly.

## splice wheel API

the splice wheel operates on the document tree with position-addressed
...
the shuttle is valuable for shuttle wheels because:
...
none of these apply to splice wheels. a splice wheel operator can hold
...
each shuttle wheel owns its own shuttle. splice wheels have no
shuttle. the loom adjusts shuttle-wheel shuttle indices when any
...
- **shuttle bends** are fine-grained: a mode + a glyph (two petals,
  10 bits). a shuttle thread is like embroidery thread — many tiny
  bends per neem.
- **splice bends** are compound: an operation + an address + content
  (variable length). a splice thread is like yarn — each bend carries
...
shuttle bends have a compact 10-bit wire format for real-time
streaming. splice bends have their own format (a tagged operation
with variable-length payload). in phase 2, when the loom moves to
tiraz, these become two API endpoints — different protocols, same
document.

## encoding

the splice wheel can work with petals directly (as shown above) or
...
concern from the document operations — the splice wheel API always
...
- shuttle-wheel panes show the shuttle cursor
- splice-wheel panes have no shuttle visualization
- authors can access each other's editing panes like scratch buffers
  (the phase 2 yank mechanism with scoop/paste)

all authors are peers. if a shuttle-wheel author likes what a
splice-wheel author wrote, they yank it — same as copying from any
other author's pane.
...
completion, spellcheck — things that help a shuttle-wheel operator
type faster by offering the next neem or phrase.

suggestions are not useful for splice-wheel operators. a splice wheel
...
loom adjusts all shuttle-wheel shuttle indices accordingly. for
example, if a splice wheel inserts a phrase above the shuttle wheel's
current position, the shuttle wheel's phrase_idx increments by one.
splice wheels need no adjustment since they specify positions explicitly
per operation.
...
shuttle and splice wheel interaction patterns. the current tape
...
only-yank-removes) apply to shuttle wheels only. splice wheels have
their own simpler semantics: direct addressing, explicit operations.

### phase 2: splice wheel implementation

the splice wheel requires the loom to be extracted into tiraz as a
separate service. shuttle wheels and splice wheels become client
connections to the same tiraz instance.
...
- develop fluency for both shuttle-wheel and splice-wheel authors

the corpus would be a collaborative effort between shuttle-wheel and
splice-wheel authors — a practical test of the multi-author editing
