...
the distinction between wheel types is about input granularity not about who is using them — a human could use a token wheel (e.g. dictation) and an animal could use a keyboard wheel

**keyboard wheels** stream bends one petal at a time with a shuttle cursor tracking position and scope within the current screed

**token wheels** operate on the document tree with position-addressed operations at neem or phrase granularity — no shuttle state is maintained as each operation specifies its target explicitly

token wheel operations include inserting, replacing, and deleting neems and phrases at specific positions in the document tree as well as reading screeds for selective traversal

a token wheel can work with petals directly or with a convenience encoding layer that maps English phonemes to pentabased glyphs

in a multi-author context each author has their own editing pane — keyboard-wheel panes show the shuttle cursor while token-wheel panes do not — and authors can access each other's panes like scratch buffers
...
- yank: {'left-shift'} push and pop nodes between the shuttle and the scratch stack (similar to vim visual / selection mode)
- swerve: {'right-shift'} manipulate the shuttle position and scope and number (similar to vim normal / navigation mode)
- cant: {'enter'} give commands to manipulate the state of the system (similar to vim command mode)

### mode sub-kinds

each mode has specific sub-kinds determined by the glyph pressed with the chord key

**loop sub-kinds:**
- loop-neem (`-`, space alone): insert a new neem at the shuttle position
- loop-phrase (`p`, space + P): insert a new phrase at the shuttle position

**yank sub-kinds:**
- delete (`-`, left-shift alone): remove the node at the current shuttle scope
- scratch buffer actions (phase 2): cut, copy, paste using right-hand keys

**swerve sub-kinds** are organized into four axes mapped to a WASD-like layout on the left hand (right-shift chord frees the left hand for glyph keys):
...
- scope axis (W/S): grow expands shuttle scope to parent node; shrink reduces to last child
- tree axis (A/D): back and forward move shuttle to previous and next sibling at current scope
- time axis (Z/X/C): undo steps backward in edit history; redo-left and redo-right navigate a binary edit history tree where forks are created by editing after undo
- selection axis (Q/E): scoop and stretch expand shuttle selection one child in either direction (phase 2)

**cant sub-kinds:**
- suggest (`-`, enter alone): accept a suggestion from tooling (e.g. word completion, type checker)
- thump (`2`, enter + '): reserved
- quit (`Q`, enter + Q): exit the wheel

in the pentabased input scheme a wheel streams a thread composed of a series of bends

a bend is the atomic unit of input on any thread — one indivisible instruction from a wheel to the loom — evoking a sailor's bend (a useful and stable configuration of a line) and a physical feature on a thread

keyboard bends and token bends differ in scale but both are bends — keyboard bends are fine-grained (a mode + a glyph, 10 bits) while token bends are compound (an operation + an address + content)

for keyboard wheels each bend encodes a single input combination of a mode (bloom/loop/yank/swerve/cant) and a glyph
...
pentabased is phonetic not orthographic — the encoding reflects pronunciation so dialectal variation is expected and welcome
...
"for" can be encoded as full `fawr` (F AO R) or reduced `f4` (F ER) depending on how it sounds in context; `ah7` (AA L) for "all" is valid in dialects with the cot-caught merger
